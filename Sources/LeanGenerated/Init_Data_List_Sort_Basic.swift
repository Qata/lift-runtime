  ...
  func merge(_ ys: List<A>) -> List<A> {
    ...
    case .cons(let head_1, let tail_2):
      ...
      case .cons(let head_3, let tail_4):
        ...
        if _x_5 {
          let _x_8: List<A> = tail_2.merge(ys)
          return List<A>.cons(head_1, _x_8)
        } else {
          let _x_6: List<A> = self.merge(tail_4)
...
@inline(__always) public func List_merge<A>(_ xs: List<A>, _ ys: List<A>) -> List<A> {
  xs.merge(ys)
}
