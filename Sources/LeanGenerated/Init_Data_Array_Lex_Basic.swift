...
public func Array_lex<A: Equatable & Equatable>(_ `as`: Array<A>, _ bs: Array<A>, _ lt: @escaping (A, A) -> Bool) -> Bool {
  let _x_2: Nat = 0
  let _x_3 = Array_size(`as`)
  let _x_4 = Array_size(bs)
  func _jp_5(_ _y_6: Nat) -> Bool {
    let _x_7 = Nat_decLt(_x_3, _x_4)
    ...
    let _x_16 = Std_Iter_toIterM(_x_15)
    let internalState_17 = _x_16.internalState
    let next_18 = internalState_17.next
    let upperBound_19 = internalState_17.upperBound
    let _x_20: Bool? = nil
    let _x_21: () = ()
    ...
    if let val_37 = next_18 {
      let _x_38 = `WellFounded_opaqueFix₃`(_f_23, val_37, _x_22)
      return _jp_8(_x_38      )
    } else {
      return _jp_8(_x_22      )
    }
