...
public func List_MergeSort_Internal_mergeTR<A>(_ `l₁`: List<A>, _ `l₂`: List<A>, _ le: @escaping (A, A) -> Bool) -> List<A> {
  let _x_1: List<A> = .`nil`
...
public func List_MergeSort_Internal_mergeTR_go<A>(_ le: @escaping (A, A) -> Bool, _ x_1: List<A>, _ x_2: List<A>, _ x_3: List<A>) -> List<A> {
  switch x_1 {
  case .`nil`:
