...
public func List_foldl<A, B>(_ f: @escaping (B, A) -> B, _ init_: B, _ xs: List<A>) -> B {
  var result = init_
  var cur = xs
  while case .cons(let head, let tail) = cur { result = f(result, head); cur = tail }
  return result
}
...
public func List_flatten<A>(_ xs: List<List<A>>) -> List<A> {
  switch xs {
  case .nil: return .nil
  case .cons(let head, let tail): return List_append(head, List_flatten(tail))
  }
}
...
public func List_append<A>(_ xs: List<A>, _ ys: List<A>) -> List<A> {
  switch xs {
  case .nil: return ys
  case .cons(let head, let tail): return .cons(head, List_append(tail, ys))
  }
}
...
// Sigma<A> in Lean has { fst : α, snd : β fst } but snd is dependent
// and not emitted. At runtime the value is carried as the constructor's
// second non-erased argument. We approximate by returning fst as Any
// since Sigma.snd projections in the generated code expect it.
// TODO: fix buildInductiveInfo to emit dependent fields as Any
